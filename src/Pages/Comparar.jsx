...
function Comparar() {
  const { compare, setCompare } = useShopify();

  const handleClose = (productId) => {
    // Filtrar los productos que tengan un ID diferente al productId
    const filteredProducts = compare.filter(
      (product) => product.id !== productId
    );
    setCompare(filteredProducts);
    localStorage.setItem("compare", JSON.stringify(filteredProducts));
  };
